...
document.addEventListener('DOMContentLoaded', () => {
    checkSession();
    if (document.getElementById('expense-items-container')) addExpenseRow();
});
...
/** 7. LOGIKA PENGELUARAN MULTI-ITEM */
...
async function simpanPengeluaran(e) {
    ...
    if (error) showCustomAlert('Gagal', error.message, 'error');
    else { showCustomAlert('Berhasil', 'Pengeluaran dicatat.'); document.getElementById('expense-items-container').innerHTML = ''; addExpenseRow(); calcExpRow(); initApp(); }
}
...
function exportToExcel() {
    ...
    const data = [
        ...
        [],
        ["DETAIL PEMASUKAN"],
        ["Tanggal", "ID Pesanan", "Menu/Item", "Metode", "Total Harga"]
    ];

    // Orders
    globalOrders.sort((a,b) => new Date(a.created_at) - new Date(b.created_at)).forEach(o => {
        const detail = o.detail_pesanan ? o.detail_pesanan.map(i => `${i.qty}x ${i.name}`).join(', ') : '';
        data.push([new Date(o.created_at).toLocaleDateString(), o.no_pesanan, detail, o.metode_pembayaran, o.total_harga]);
    });

    // Expenses
    data.push([], ["DETAIL PENGELUARAN"], ["Tanggal", "Nama Item", "Qty", "Harga Satuan", "Total", "Metode"]);
    globalExpenses.forEach(e => {
        data.push([e.tanggal, e.nama_item, e.qty, e.harga_satuan, e.total, e.metode_pembayaran]);
    });
...
function exportToPDF() {
    ...
    // Header Branding
    doc.setFillColor(23, 12, 121); // #170C79
    ...
    doc.autoTable({
        ...
        theme: 'striped',
        headStyles: { fillColor: [86, 182, 198] } // #56B6C6
    });
    ...
    doc.autoTable({
        ...
        head: [['Tanggal', 'ID', 'Item', 'Metode', 'Total']],
        body: globalOrders.map(o => [
            new Date(o.created_at).toLocaleDateString(), 
            o.no_pesanan, 
            o.detail_pesanan.map(i => `${i.qty}x ${i.name}`).join('\n'), 
            o.metode_pembayaran, 
    ...
    doc.autoTable({
        ...
        body: globalExpenses.map(e => [e.tanggal, e.nama_item, e.qty, `Rp ${e.total.toLocaleString()}`]),
